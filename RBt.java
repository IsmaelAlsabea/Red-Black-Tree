...
class RBt {
	...
	void rbDeletionColorHandler(Node node) {
		if (node.color == c.RED /* || node.parent.color==c.RED */ || hasARedChild(node))
			firstCaseRBDeletionColorHandler(node);
		else if (node.color == c.BLACK)// && node.parent.color==c.BLACK)
		{
			// node.color=c.doubleBlack;
	...
	private void firstCaseRBDeletionColorHandler(Node nodeX) {
		...
		// no rotations since the node that was deleted was red.
		// else if (nodeX.color==c.BLACK && nodeX.parent.color==c.RED)
		// nodeX.parent.color=c.BLACK;
	}
	...
	private Node getUncle(Node f) {
		...
		/*
		 * if (p.key > p.parent.key) return p.parent.getLeft(); else if (p.key <
		 * p.parent.key) return p.parent.getRight(); else return null;
		 */
